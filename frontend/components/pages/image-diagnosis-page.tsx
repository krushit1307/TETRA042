...
import {
  ...
  type VoiceCallMode,
} from "@/components/voice/voice-call-overlay"
...
export default function ImageDiagnosisPage() {
  ...
  const [voiceCallMode, setVoiceCallMode] = useState<VoiceCallMode>("connecting")
  const [isStartingCall, setIsStartingCall] = useState(false)
  const conversationRef = useRef<any>(null)
  const shouldEndCallRef = useRef(false)

  const getAudioLevels = useCallback((): VoiceAudioLevels => {
    ...
    try {
      return {
        ...
        inputFrequency: conversation.getInputByteFrequencyData?.() ?? null,
        outputFrequency: conversation.getOutputByteFrequencyData?.() ?? null,
      }
    } catch {
      return {
        inputVolume: 0,
  ...
  const startVoiceCall = useCallback(async () => {
    if (!diagnosis || isStartingCall) return
    ...
    try {
      ...
      const sessionObject = await apiClient.startVoiceSession({
        language: selectedLanguage,
        crop: diagnosis.crop,
        disease: diagnosis.disease,
        confidence: diagnosis.confidence,
      ...
      if (!signedUrl) {
        throw new Error("Voice assistant is unavailable — signed URL could not be generated.")
      }
      ...
      const sessionOptions: any = {
        ...
        onModeChange: async ({ mode }: { mode: "speaking" | "listening" }) => {
          setVoiceCallMode(mode)

          if (shouldEndCallRef.current && mode === "listening" && conversationRef.current) {
            shouldEndCallRef.current = false
        ...
        onMessage: (messagePayload: any) => {
          const isUser = messagePayload.role === "user" || messagePayload.source === "user"
      ...
      console.error("Voice call connection failed:", err)
      setVoiceCallOpen(false)
      setVoiceCallMode("connecting")
      const errMsg = err?.message || "Failed to connect to agent"
      toast.error(`Connection Failed: ${errMsg}`)
    } finally {
      setIsStartingCall(false)
    }
  }, [diagnosis, isStartingCall, selectedLanguage])
